...
pub fn puzzle1() {
  let (wire1, wire2) = get_inputs();
  println!("day3 puzzle1: {}", calc1(wire1, wire2));
}

pub fn puzzle2() {
  let (wire1, wire2) = get_inputs();
  println!("day3 puzzle2: {}", calc2(wire1, wire2));
}

fn get_inputs() -> (&'static str, &'static str) {
  return (
...
fn calc1(wire1: &str, wire2: &str) -> i32 {
  let wire1locs = process_wire(wire1);
  let wire2locs = process_wire(wire2);

  let mut intersections: Vec<i32> = vec![];

  for ((x, y), _v) in wire2locs {
    if wire1locs.contains_key(&(x, y)) {
...
fn calc2(wire1: &str, wire2: &str) -> i32 {
  let wire1locs = process_wire(wire1);
  let wire2locs = process_wire(wire2);

  let mut intersections: Vec<i32> = vec![];

  for (k, v) in wire2locs {
    if let Some(v2) = wire1locs.get(&k) {
      intersections.push(v + v2);
    }
  }
...
fn process_wire(mvs: &str) -> HashMap<(i32, i32), i32> {
    ...
    match iter.next() {
      Some('U') => {
        ...
        for _ in 0..n {
          steps += 1;
          y += 1;
          if !locs.contains_key(&(x, y)) {
            locs.insert((x, y), steps);
          }
        }
      }
      Some('R') => {
...
fn test_calc2() {
  let wire1 = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
  let wire2 = "U62,R66,U55,R34,D71,R55,D58,R83";
  assert_eq!(calc2(wire1, wire2), 610);

  let wire1 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
  let wire2 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";
  assert_eq!(calc2(wire1, wire2), 410);
}
